...
module.exports = (server: any) => {
    ...
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('enter public room', (username: string) => {
            User.findOne({ id: username }, (err: any, user: { _id: any; }) => {
                ...
                if (!user) {
                    ...
                    PublicRoom.findOne({ username: username }, (err: any, participant: { socketId: string; createdAt: string; save: (arg0: (err: any) => void) => void; }) => {
                        if (err) throw err;
                        if (!participant) {
        ...
        function getClientList() {
            PublicRoom.find({}).exec((err: any, user: Array<publicRoomType>) => {
                if (err) throw err;
        ...
        // 귓속말 채팅 보내기
        socket.on('private send message', (from: string, to: string, msg: string) => {
            // 두 유저가 존재하는 유저인지 확인
